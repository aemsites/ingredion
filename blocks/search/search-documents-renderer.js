...
const API_CONFIG = {
  ...
  searchPath: '/content/ingredion-com/na/en-us/search/jcr:content/searchResults.techDocs.json',
};
...
async function updateUrlAndFetchResults(url, context, resetToFirstPage = true) {
  try {
    ...
    context.techDocsResults = {
      ...newResults,
      appliedFacets: context.techDocsResults.appliedFacets,
    };
...
/**
 * Create an DocumentRenderer.
 * @param {string} options.techDocsResults - array of filtered ingredient results.
...
export default class DocumentRenderer {
  constructor({
    ...
    this.techDocsResults = techDocsResults;
    ...
    Array.from(url.searchParams.entries()).forEach(([group, values]) => {
      ...
      valueArray.forEach((value) => {
        ...
        const facetGroup = techDocsResults.facets?.[group];
    ...
    this.state = {
      allArticles: techDocsResults.results,
      totalArticles: techDocsResults.totalItemsCount,
  ...
  renderFilterTags() {
    ...
    const createFacetGroup = (facetKey, facetData) => {
      const isActive = this.techDocsResults.appliedFacets?.some((f) => f.group === facetKey);
      ...
      facetData.options.forEach((option) => {
        const isSelected = this.techDocsResults.appliedFacets?.some(
        ...
        checkbox.addEventListener('change', async () => {
          ...
          try {
            ...
            if (filter.checked) {
              this.techDocsResults.appliedFacets = [
                ...(this.techDocsResults.appliedFacets || []),
                {
                  group: filter.group,
                  value: filter.value,
              ...
              this.techDocsResults.appliedFacets = (this.techDocsResults.appliedFacets || [])
    ...
    // Add facet groups
    Object.entries(this.techDocsResults.facets || {}).forEach(([key, data]) => {
  ...
  async render() {
    try {
      const data = this.techDocsResults.results;
      this.state.allArticles = data;
      this.state.totalArticles = this.techDocsResults.totalItemsCount;
